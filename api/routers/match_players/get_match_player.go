...
func GetMatchPlayer(request events.APIGatewayProxyRequest) dto.RestResponse {
	...
	match, _, err := match_players_service.GetMatchPlayer(id)
	if err != nil {
		response.Status = http.StatusNotFound
		response.Message = "Error to get match player: " + err.Error()
		return response
	}
