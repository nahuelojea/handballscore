...
func ProcessRequest(ctx context.Context, request events.APIGatewayProxyRequest, claim dto.Claim, response dto.RestResponse) dto.RestResponse {
	...
	case "GET":
		...
		case "coach/filter":
			return coaches.GetCoachs(request)
		case "coach/avatar":
			return coaches.GetAvatar(ctx, request)
		}
	case "PUT":
		switch ctx.Value(dto.Key("path")).(string) {
		case "coach":
		...
		case "coach/avatar":
			return coaches.UpdateAvatar(ctx, request)
		}
	case "DELETE":
		switch ctx.Value(dto.Key("path")).(string) {
		case "coach":
			return coaches.DisableCoach(request)
		}
	}
